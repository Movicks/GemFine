    ...
    <div className="flex flex-col gap-1 bg-transparent">
      ...
      <div className="h-[2.8rem] border-2 border-red-300 flex items-center gap-3 px-4 rounded-full shadow-custom">
        ...
        <input
          ...
          {...register("password")}
          className="custom-input bg-transparent outline-none h-full w-full border-0 hover:border-0 active:border-0 focus:border-0"
          style={{ border: "none", outline: "none" }}
        />
        <div onClick={togglePasswordVisibility} className="cursor-pointer">
          {showPassword ? (
            <ImEyeBlocked className="text-xl text-red-300" />
          ) : (
            <ImEye className="text-xl text-red-300" />
          )}
        </div>
      </div>
    </div>

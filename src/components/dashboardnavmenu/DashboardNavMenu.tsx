...
const DashboardNavMenu = ({ onWidthChange }: DashboardNavMenuProps) => {
  ...
  return (
    <div
      ...
      className="BottomFooter fixed z-100 left-0 bottom-0 h-[4.5rem] shadow-custom py-2 md:pr-1 overflow-hidden w-full bg-white md:w-auto md:flex-col md:items-start md:fixed md:top-0 md:left-0 md:h-full"
    >
      <div className="relative flex items-center md:pb-3 md:pl-4 justify-between md:max-h-[21rem] overflow-hidden w-full md:h-full md:flex-col md:items-start">
        ...
        >
          <FaHome
            className={` ${isMdScreen && showIconsOnly ? "text-2xl" : "text-2xl"}`}
          />
          {(!isMdScreen || !showIconsOnly) && (
            <span
              className={`${
                activeLink === "/"
